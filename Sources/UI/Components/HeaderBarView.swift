...
struct HeaderBarView: View {
    ...
    var body: some View {
        ...
        .padding(.top, 12)
        .padding(.bottom, UISpec.contentVerticalPadding)
    }
    ...
    private func githubLogoImage() -> NSImage? {
        // Priority 1: bundled resource.
        if let url = Bundle.main.url(forResource: "github-mark", withExtension: "png"),
           let image = NSImage(contentsOf: url) {
            return image
        }

        // Priority 2: embedded fallback (downloaded GitHub mark), guarantees local debug visibility.
        let embeddedBase64 = """
...
"""
        guard let data = Data(base64Encoded: embeddedBase64),
